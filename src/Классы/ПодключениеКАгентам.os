...
Функция ОписанияОбъектовКластера(Знач ТипОбъекта, Знач Обновить = Ложь,
	...
	Для Каждого ТекОписаниеОбъекта Из КэшОписанийОбъектов Цикл
		...
		ОписаниеОбъектаДляВывода = ОбновленныеОписания[ТекОписаниеОбъекта.Значение[ТипОбъекта]];
...
Функция ПолныеОписанияКластеровАгента(Знач ИдАгента)

	Агент = Агенты[ИдАгента].Агент;
	...
	Для Каждого ТекКластер Из СписокКластеров Цикл
		...
		ДополнитьОписаниеОбъектаКластера(ПолноеОписаниеОбъекта, ТипыОбъектовКластера().Кластеры, ИдАгента, ТекКластер);
...
Функция ПолныеОписанияОбъектовАгента(Знач ИдАгента, Знач ТипОбъекта)

	Если ВРег(ТипОбъекта) = ВРег(ТипыОбъектовКластера().Кластеры) Тогда
		Возврат ПолныеОписанияКластеровАгента(ИдАгента);
	КонецЕсли;
	...
	Для Каждого ТекКластер Из СписокКластеров Цикл
		...
		ПолныеОписанияОбъектовКластера = ПолныеОписанияОбъектовКластера(ТекКластер, ТипОбъекта, ИдАгента);
...
Функция ПолныеОписанияОбъектовКластера(Знач Кластер, Знач ТипОбъекта, Знач ИдАгента)
	...
	Для Каждого ТекОбъектКластера Из СписокОбъектовКластера Цикл
		...
		ДополнитьОписаниеОбъектаКластера(ПолноеОписаниеОбъекта, ТипОбъекта, ИдАгента, Кластер);
...
Процедура ДополнитьОписаниеОбъектаКластера(ПолноеОписаниеОбъекта, ТипОбъекта, ИдАгента, Кластер)

	ПолноеОписаниеОбъекта.Вставить("agent", ИдАгента);
...
Процедура ЗаполнитьРасширенноеОписаниеОбъекта(ТипОбъекта, ПолноеОписаниеОбъекта, Поля = "_all", ОбновленныеОписания)
	...
	Агент = Агенты[ВРег(ПолноеОписаниеОбъекта["agent"])].Агент;

	Кластер = ПолноеОписаниеОбъекта["_clusterObject"];
	Если Кластер = Неопределено Тогда
		Кластер = Агент.Кластеры().Получить(ПолноеОписаниеОбъекта[ТипыОбъектовКластера().Кластеры]);
		ПолноеОписаниеОбъекта.Вставить("_clusterObject", Кластер);
	КонецЕсли;
	
	ОбъектКластера = ПолноеОписаниеОбъекта["_thisObject"];
	Если ОбъектКластера = Неопределено Тогда
		Если ТипОбъекта = ТипыОбъектовКластера().ИнформационныеБазы Тогда
			ОбъектКластера = Кластер.ИнформационныеБазы().Получить(ПолноеОписаниеОбъекта[ТипОбъекта]);
		ИначеЕсли ТипОбъекта = ТипыОбъектовКластера().Соединения Тогда
			ОбъектКластера = Кластер.Соединения().Получить(ПолноеОписаниеОбъекта[ТипОбъекта]);
		КонецЕсли;
		ПолноеОписаниеОбъекта.Вставить("_thisObject", ОбъектКластера);
	КонецЕсли;
...
Процедура ОбновитьКэшОписанийОбъектовКластеров(Знач ТипОбъекта, Знач Обновить = Ложь)
	...
	Если ПрочитатьКэшИзФайла(ТипОбъекта) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ТекАгент Из Агенты Цикл
		...
		ОписанияОбъектовАгента = ПолныеОписанияОбъектовАгента(ТекАгент.Ключ, ТипОбъекта);
	...
	Если Настройки.ВремяЖизниКэша() > 0 Тогда
		СохранитьКэшВФайл(ТипОбъекта);
	КонецЕсли;
...
Процедура СохранитьКэшВФайл(Знач ТипОбъекта) Экспорт

	Если ФайлКэшаЗаблокирован(ТипОбъекта) Тогда
		Возврат;
	КонецЕсли;

	ПрефиксФайлов = ПрефиксФайловКэша(ТипОбъекта);

	ПутьКФайлуКэша  = ОбъединитьПути(Настройки.ПутьККаталогуКэша(), СтрШаблон("%1.json", ПрефиксФайлов));

	КэшОписаний = КэшОписанийОбъектовКластера(ТипОбъекта);

	КэшДляСохранения = Новый Соответствие();
	Для Каждого ТекЭлемент Из КэшОписаний Цикл
		КэшДляСохранения.Вставить(ТекЭлемент.Ключ, ТекЭлемент.Значение);
		КэшДляСохранения[ТекЭлемент.Ключ].Вставить("_thisObject", Неопределено);
		КэшДляСохранения[ТекЭлемент.Ключ].Вставить("_clusterObject", Неопределено);
	КонецЦикла;

	Если ЗаблокироватьФайлКэша(ТипОбъекта) Тогда

		ОбновитьАктуальностьКэша(ТипОбъекта);

		Запись = Новый ЗаписьJSON();
		Запись.ОткрытьФайл(ПутьКФайлуКэша, "UTF-8", Ложь, Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Нет));
		Попытка
			ЗаписатьJSON(Запись, КэшДляСохранения);
			Запись.Закрыть();
		Исключение
		КонецПопытки;
		
		РазблокироватьФайлКэша(ТипОбъекта);
	КонецЕсли;

КонецПроцедуры // СохранитьКэшВФайл()

Функция ПрочитатьКэшИзФайла(Знач ТипОбъекта) Экспорт

	Если НЕ ФайлКэшаАктуален(ТипОбъекта) Тогда
		Возврат Ложь;
	КонецЕсли;

	Если НЕ ЗаблокироватьФайлКэша(ТипОбъекта) Тогда
		Возврат Ложь;
	КонецЕслИ;

	ПутьККаталогу = Настройки.ПутьККаталогуКэша();

	ПрефиксФайлов = ПрефиксФайловКэша(ТипОбъекта);

	ПутьКФайлуКэша  = ОбъединитьПути(ПутьККаталогу, СтрШаблон("%1.json", ПрефиксФайлов));

	Файл = Новый Файл(ПутьКФайлуКэша);
	...
	Чтение.ОткрытьФайл(ПутьКФайлуКэша, "UTF-8");

	КэшОписанийОбъектов = ПрочитатьJSON(Чтение, Истина);

	Если ВРег(ТипОбъекта) = ВРег(ТипыОбъектовКластера().Кластеры) Тогда
		ОписанияКластеров = КэшОписанийОбъектов;
	ИначеЕсли ВРег(ТипОбъекта) = ВРег(ТипыОбъектовКластера().Серверы) Тогда
		ОписанияСерверов = КэшОписанийОбъектов;
	ИначеЕсли ВРег(ТипОбъекта) = ВРег(ТипыОбъектовКластера().РабочиеПроцессы) Тогда
		ОписанияРабочихПроцессов = КэшОписанийОбъектов;
	ИначеЕсли ВРег(ТипОбъекта) = ВРег(ТипыОбъектовКластера().ИнформационныеБазы) Тогда
		ОписанияИнформационныхБаз = КэшОписанийОбъектов;
	ИначеЕсли ВРег(ТипОбъекта) = ВРег(ТипыОбъектовКластера().Сеансы) Тогда
		ОписанияСеансов = КэшОписанийОбъектов;
	ИначеЕсли ВРег(ТипОбъекта) = ВРег(ТипыОбъектовКластера().Соединения) Тогда
		ОписанияСоединений = КэшОписанийОбъектов;
	КонецЕсли;
...
КонецФункции // ПрочитатьКэшИзФайла()

Функция ФайлКэшаАктуален(Знач ТипОбъекта) Экспорт
	...
	ПутьКФайлуМетки = ОбъединитьПути(Настройки.ПутьККаталогуКэша(), СтрШаблон("%1.tmstp", ПрефиксФайлов));

	Файл = Новый Файл(ПутьКФайлуМетки);
	Если НЕ (Файл.Существует() И Файл.ЭтоФайл()) Тогда
		Возврат Ложь;
	КонецЕсли;

	Текст = Новый ТекстовыйДокумент();
	Текст.Прочитать(ПутьКФайлуМетки);

	Метка = Текст.ПолучитьСтроку(1);
	
	Возврат (ТекущаяУниверсальнаяДатаВМиллисекундах() - Число(Метка)) / 1000 < Настройки.ВремяЖизниКэша();

КонецФункции // ФайлКэшаАктуален()

Процедура ОбновитьАктуальностьКэша(Знач ТипОбъекта) Экспорт
	...
	Текст = Новый ТекстовыйДокумент();
	Текст.ДобавитьСтроку(Формат(ТекущаяУниверсальнаяДатаВМиллисекундах(), "ЧРД=.; ЧН=; ЧГ=0"));
	Текст.Записать(ПутьКФайлуМетки);

КонецПроцедуры // ОбновитьАктуальностьКэша()

Функция ФайлКэшаЗаблокирован(Знач ТипОбъекта) Экспорт

	ПрефиксФайлов = ПрефиксФайловКэша(ТипОбъекта);

	ПутьКФайлуБлокировки = ОбъединитьПути(Настройки.ПутьККаталогуКэша(), СтрШаблон("%1.lock", ПрефиксФайлов));

	Файл = Новый Файл(ПутьКФайлуБлокировки);

	Возврат Файл.Существует() И Файл.ЭтоФайл();

КонецФункции // ФайлКэшаЗаблокирован()

Функция ЗаблокироватьФайлКэша(Знач ТипОбъекта) Экспорт
	...
	Попытка
		Файл.Записать(ПутьКФайлуБлокировки);
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
КонецФункции // ЗаблокироватьФайлКэша()

Процедура РазблокироватьФайлКэша(Знач ТипОбъекта) Экспорт

	Если НЕ ФайлКэшаЗаблокирован(ТипОбъекта) Тогда
		Возврат;
	КонецЕсли;
	...
	УдалитьФайлы(ПутьКФайлуБлокировки);

КонецПроцедуры // РазблокироватьФайлКэша()

Функция ПрефиксФайловКэша(Знач ТипОбъекта) Экспорт

	Возврат СтрШаблон("iracache_%1", ТипОбъекта);

КонецФункции // ПрефиксФайловКэша()
