...
Функция Список(Знач ТипОбъектов = Неопределено) Экспорт
	...
	Для Каждого ТекРаздел Из Настройки.ПараметрыСчетчиков() Цикл
...
Функция Счетчики(Знач Объекты,
	...
	ПараметрыСчетчиковОбъекта = Настройки.ПараметрыСчетчиков()[ТипОбъектов];
	...
	Если ВРег(ФорматСчетчиков) = ОбщегоНазначения.ФорматыРезультата().prometheus Тогда
		Возврат ФорматPrometheus(Счетчики, ПрефиксСчетчиков);
	ИначеЕсли ВРег(ФорматСчетчиков) = ОбщегоНазначения.ФорматыРезультата().plain Тогда
