...
Функция НачатьЗамерКоманды(Команда) Экспорт

	КомандаРодитель = "";
	Если Команда.КомандыРодители.Количество() > 0 Тогда
		КомандаРодитель = Команда.КомандыРодители[Команда.КомандыРодители.ВГраница()].ПолучитьИмяКоманды();
	КонецЕсли;

	ПараметрыЗамера = НачатьЗамер(СтартовыйСценарий().Источник,
...
КонецФункции // НачатьЗамерКоманды()
